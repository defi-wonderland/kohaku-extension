...
- **`addApproverReply`** checks the pasted reply's shape first and never throws: a record that is not a reply or whose fields are not the record's types is `version-unread`; then `binding-mismatch`, `place-unknown`, `credential-mismatch`, `digest-mismatch`. The seam's `verifyReply` answers `rejected` to a malformed paste.
