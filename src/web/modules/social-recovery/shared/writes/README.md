...
| `classify.ts` | Receipts and failures (`receiptOf`, `writeFailureOf`), the causes of a revert (`revertCauseOf`), what a retry can fix (`NO_RETRY_CAUSES`, `retryCanFix`, `leavesAttemptReady`), `classifyFailure` and `settleReceipt`. |
| `machine.ts` | `writeReducer`, the one pure reducer over the states, its events and `initialWriteState`. |
| `gas.ts` | `checkGas`, the deposit step's data (`DepositStep`, `DepositRoute`), the estimate (`gasEstimateOf`, `FEE_HEADROOM_PERCENT`) and the transfer route's own fee (`transferTransactionOf`, `transferFeeOf`). |
| `copy.ts` | The en.json keys the lane reads (`WRITES_KEYS`, `GAS_KEYS`, `REVERTED_KEYS`, `revertedKeyOf`, `causeKey`, `cancelGoneRoadKey`), `renderWriteState` and `renderDepositStep`. |
...
| `gasReadError` | A read of the gas check could not run (a `ProviderReadFailure`): its own line with the retry, never the not-sent reading (D-393). It is part of the gas check, not a reading of the failed state. |
...
`start` from a state that offers the retry (`canRetry`) runs the gas check again, at that moment's fee. An event a state does not take leaves the state as it was, the same object, so a late answer of an earlier run never moves the screen. `attemptRead` judges a reverted cancel again once the attempt read returns (below).
...
`writeFailureOf(thrown)` finds a replacement's reason, a receipt (ethers' `CALL_EXCEPTION` from `wait()`) or a hash (`transactionHash`, `hash`, `transaction.hash`) on a thrown value. The reducer's `error` event runs it, so an error that carries its reverted receipt settles as a revert.
...
- `attemptGone`: the owner's cancel reverted because the attempt was already gone (D-307). It is decided on one of three grounds, never from a revert the wallet could not decode:
  - the attempt read after the revert (`attemptAfter`, `{ ended, controller? }`) says the attempt ended. After an execution the state names the account's controller as it now stands and offers the cancel's move-funds action. After another road (`cancelByOwner`, `cancelByProofs`, `cancelByVeto`, `setupWrite`) it names that road, with control unchanged and no move-funds action;
...
| Write | Kit errors no retry fixes |
| --- | --- |
| `save`, `edit` | `InvalidCommitment` |
| `ownerWrite` | `NoSetup` |
| `cancel` | none; the gone attempt already offers no retry |
| `submission` | the acceptance errors: `NoSetup`, `AttemptAlreadyActive`, `WrongAttemptId`, `WrongSetupNonce`, `SetupCommitmentMismatch`, `StaleAttempt`, `PlaceOutOfRange`, `CredentialMismatch`, `PlacesNotStrictlyIncreasing`, `RequestExpired`, `ProofRejected`, `MethodStopped`, `RuleUnsatisfied`, `MalformedHandover`, `ReservedAuthority` |
| `execution` | every kit error but `EXECUTION_STILL_READY_CAUSES` (`WaitNotOver`, `MethodVetoedSpend`): the fifth ending of D-393 offers no retry |
...
`revertedKeyOf(write, cause)` picks the reverted reading by the write kind, from each write's own frame (`REVERTED_KEYS`), and for the execution by the cause. Every one carries the cause as `{{cause}}`.
...
| `execution` | `revertedExecute` ("still ready") for `WaitNotOver`, `MethodVetoedSpend` or a cause the wallet cannot name; `revertedExecuteGone` for every other cause | D-13 |
...
- The decoded cause of a revert comes from the consumer, which replays the call and decodes it through the SDK. The SDK surface of this repository declares the decoded shape (`KitError`) but no decoder member. The attempt read after a cancel's revert is the consumer's too, sent with the `attemptRead` event.
- `FEE_HEADROOM_PERCENT`, `VALUE_TRANSFER_GAS` and the six-decimal rounding are the extension's own estimation choices; no chapter sentence fixes them.
