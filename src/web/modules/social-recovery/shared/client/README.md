...
1. The facade dispatches `REQUESTS_CONTROLLER_ADD_USER_REQUEST` with a `SignUserRequest` (`signRequestOf`): an id of its own that no other page makes (`social-recovery-signer:` and a random UUID; the queue takes a string id), an internal session (`new Session({ windowId })`, origin `internal`, no dApp), `meta: { isSignAction: true, accountAddr, keyType, chainId }`, the `typedMessage` or `message` content, and `allowAccountSwitch: true`. `accountAddr` is the listed account's own address as the wallet holds it (checksum-cased), not the caller's spelling, since the queue and the sign-message controller compare addresses with exact case. `keyType` carries the handle's key type with the request (D-370 addresses the signer by address and key type), so a later change to the action window can honour it.
...
4. The facade takes the signature from the pushed `signMessage` state whose `signedMessage.fromActionId` is its request's id. The background pushes that state before it removes the request from the queue. The facade then verifies the signature in its own page (`recoveredSignerOf`): ethers' `verifyTypedData` over its own typed message, or `verifyMessage` over its own bytes, must recover the key's address. A signature that cannot be recovered is refused as `malformed-signature`, and one that recovers to another address as `signer-mismatch`.
...
6. With no answer in 10 minutes (`DEFAULT_SIGN_TIMEOUT_MS`), the facade withdraws its request with `REQUESTS_CONTROLLER_REMOVE_USER_REQUEST` and rejects. That removal reaches a request in the queue's `userRequests` alone. A request that still waits for an account switch stays in `userRequestsWaitingAccountSwitch`, which only the action window's close clears (item 7).
...
7. The queue shows one sign-message request at a time. A request added while another one is visible is dropped without a trace in the pushed state, and the facade waits until its timeout. The queue also skips a request while a hardware signing is in progress, with the same outcome. A timeout cannot remove a request that waits for an account switch: `REQUESTS_CONTROLLER_REMOVE_USER_REQUEST` looks in `userRequests` alone, and only the action window's close clears `userRequestsWaitingAccountSwitch`. If the holder confirms the switch after the facade gave up, the sign request still opens; a signature given then reaches no caller, and the background still records it in the activity.
...
10. A queued request that returns no signature rejects with a `SignFlowFailure`: `refused` (the request left the queue unsigned), `timeout` (the facade withdrew it), `malformed-signature` (not hex, or not recoverable over the facade's content) or `signer-mismatch` (it recovers to another address than the key's).
...
4. The queue's gaps for a wallet-originated sign request: no result channel of its own (item 9), a silent drop while another sign-message request is visible (item 7), no removal of a request that waits for an account switch, so a timeout leaves it in `userRequestsWaitingAccountSwitch` until the action window closes (item 7), and the requester line "The App is requesting your signature" for a request no dApp made.
